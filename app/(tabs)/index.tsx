...
export default function Index() {
  ...
  useEffect(() => {
    ...
    const completionChannel = `databases.${DATABASE_ID}.collections.${COMPLETED_COLL_ID}.documents`;
  ...
  const fetchTodayCompletions = async () => {
    try {
      ...
      const res = await database.listDocuments(DATABASE_ID, COMPLETED_COLL_ID, [
        Query.equal("user_id", user?.$id || ""),
        Query.greaterThanEqual("completed_at", today.toISOString()),
      ]);
      const completions = res.documents as unknown as CompletedHabit[];
  ...
  const isHabitComplete = (id: string) => completedHabits?.includes(id);
          ...
          habits?.map((habit, key) => (
            ...
            >
              <Surface
                style={[
                  styles.card,
                  isHabitComplete(habit.$id) && styles.cardCompletion,
                ]}
                elevation={0}
              >
